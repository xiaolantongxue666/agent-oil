...
async function discover(row: TeacherPositionOut | null = null, lookbackMonths = 0) {
  ...
  await ElMessageBox.confirm(
    ...
    isBackfill ? '近6个月历史回补' : 'AI寻找岗位数据',
    { confirmButtonText: isBackfill ? '开始回补' : '开始寻找', cancelButtonText: '取消', type: 'warning' },
  )
  actionId.value = positionId
  try {
    const result = await teacherApi.discoverPosition(positionId, isBackfill ? 50 : 20, lookbackMonths)
    const officialStatusText = result.official_sources
      .map((item) => `${item.source}：${item.detail}`)
      .join('\n')
    if (result.saved_count > 0) {
      const officialCount = result.stage_stats.official_relevant_positions || 0
      if (officialCount > 0) {
        ElMessage.success(
      ...
      <el-table :data="positions" stripe>
        ...
        <el-table-column label="操作" width="280" fixed="right">
          <template #default="{ row }">
            ...
            <el-button text type="primary" :loading="actionId === row.id" @click="discover(row)">AI寻找</el-button>
      ...
      <div v-if="detail" class="drawer-content" v-loading="actionId === detail.position.id">
        <section class="summary-panel">
          ...
          <div class="workflow-actions">
            <el-button type="primary" plain @click="discover()">1. AI寻找近期岗位</el-button>
